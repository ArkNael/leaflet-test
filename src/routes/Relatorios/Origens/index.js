...
const Origens = props => {
	...
	useEffect(() => {
		const getData = async () => {
			api.get('api/relatorios/origens')
			.then(({data}) => {
				let newData = transformData(data, 'origens')
	...
	return (
		<Card
			className="gx-card" 
			type="inner" 
			title={ <h2 className="title gx-mb-4"><IntlMessages id={`sidebar.${props.controller}.origens`} /></h2> }
		>
